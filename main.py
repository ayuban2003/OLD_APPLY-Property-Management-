    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            if len(apmtname) > 0 and len(plot_no) > 0 and len(address) > 0 and len(landmark) > 0 and len(availability) and len(
                city) > 0 and len(pin) > 0 and len(Province) > 0 and len(country) > 0 and len(rs)>0 and len(
                atype) > 0 and len(area)>0 and len(facilities) > 0 and len(description) > 0 and len(Price)>0:

                if len(pin) != 4:
                    msg = 'Enter 4 digit Zipcode !'
                elif not apmtname or not plot_no or not area or not address or not landmark or not city or not pin or not Province or not country or not atype or not rs or not availability or not Price or not facilities or not description or not file:
                    msg = 'Please fill out the form !'
                else:
                    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                    cursor.execute(
                        ...
                        (
                            session['id'] , apmtname, plot_no, area, address, landmark, city, pin, Province, country, atype, rs, availability,
    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            if len(bname) > 0 and len(room_no) > 0 and len(area) and len(address) > 0 and len(landmark) > 0 and len(
                city) > 0 and len(pin) > 0 and len(Province) > 0 and len(country) > 0 and len(availability) and len(
                Rent)>0 and len(facilities) > 0 and len(description) > 0:

                if len(pin) != 4:
                    msg = 'Enter 4 digit Zipcode !'
                elif not bname or not room_no or not area or not address or not landmark or not city or not pin or not Province or not country or not availability or not Rent or not facilities or not description or not file:
                    msg = 'Please fill out the form !'
                else:
                    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                    cursor.execute(
                        ...
                        (
                            session['id'], bname, room_no, area, address, landmark, city, pin, Province, country, availability,
    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            if len(pname) > 0 and len(flattype) > 0 and len(address) > 0 and len(features) > 0 and len(
                city) > 0 and len(pin) > 0 and len(Province) > 0 and len(country) > 0 and len(
                availability) > 0 and len(facilities) > 0 and len(description) > 0:

                if len(pin) != 4:
                    msg = 'Enter 4 digit Zipcode !'
                elif not pname or not flattype or not features or not address or not city or not pin or not Province or not country or not availability or not facilities or not description or not file:
                    msg = 'Please fill out the form !'
                else:
                    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                    cursor.execute(
                        ...
                        (
                            session['id'], pname, flattype, address, features, city, pin, Province, country, availability,
...
def search():
    ...
    if request.method == 'POST':
        ...
        if option == "choose" :
            ...
            if loc == "" and city == "" :
                cur.execute(
                    'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id')
                result = cur.fetchall()
            elif loc != "":
                if city == "" :
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s',
                        [loc])
                    result = cur.fetchall()
                elif city != "" :
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s',
                        ([loc], [city]))
                    result = cur.fetchall()
            elif city != "" and loc == "":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s',
                        [city])
                    result = cur.fetchall()
            ...
            if loc == "" and city == "" :
                cur.execute(
                    'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id')
                result1 = cur.fetchall()
            elif loc != "":
                if city == "" :
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s',
                        [loc])
                    result1 = cur.fetchall()
                elif city != "" :
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City = %s',
                        ([loc], [city]))
                    result1 = cur.fetchall()
            elif city != "" and loc == "":
                if minprice == "" and maxprice == "":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s',
                        [city])
                    result1 = cur.fetchall()
            ...
            if loc == "" and city == "":
                cur.execute(
                    'SELECT P_ID,username,Pname,Flattype,Features,Address,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id')
                result2 = cur.fetchall()
            elif loc != "":
                if city == "":
                    cur.execute(
                        'SELECT P_ID,username,Pname,Flattype,Features,Address,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id where Province = % s',
                        [loc])
                    result2 = cur.fetchall()
                elif city != "":
                    cur.execute(
                        'SELECT P_ID,username,Pname,Flattype,Features,Address,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id where Province = % s and City = %s',
                        ([loc], [city]))
                    result2 = cur.fetchall()
            elif city != "" and loc == "" :
                cur.execute(
                    'SELECT P_ID,username,Pname,Flattype,Features,Address,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id where City = % s',
                    [city])
                result2 = cur.fetchall()
        ...
        if option == "apartments":
            ...
            if loc == "" and city == "" and minprice == "" and maxprice == "" and area== "" and atype=="" :
                ...
                cur.execute(
                    'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Area=%s',[area])
                result = cur.fetchall()
            elif loc == "" and city == "" and minprice == "" and maxprice == "" and area== "" and atype!="" :
                cur.execute(
                    'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Atype=%s',[atype])
                result = cur.fetchall()
            elif loc == "" and city == "" and minprice == "" and maxprice == "" and area!= "" and atype!="" :
                cur.execute(
                    'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Area=%s and Atype=%s',([area], [atype]))
                result = cur.fetchall()
                ...
                if city == "" and minprice == "" and maxprice == "" and area=="" and atype=="":
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Area=%s',
                        ([loc],[area]))
                    result = cur.fetchall()
                elif city == "" and minprice == "" and maxprice == "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Atype=%s',
                        ([loc],[atype]))
                    result = cur.fetchall()
                elif city == "" and minprice == "" and maxprice == "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Atype=%s and Area=%s',
                        ([loc],[atype],[area]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Area=%s',
                        ([loc], [city], [area]))
                    result = cur.fetchall()
                elif city != "" and minprice == "" and maxprice == "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Atype=%s',
                        ([loc], [city], [atype]))
                    result = cur.fetchall()
                elif city != "" and minprice == "" and maxprice == "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Atype=%s and Area=%s' ,
                        ([loc], [city], [atype], [area]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price>=%s',
                        ([loc], [city], [minprice]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice == "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price>=%s and Area=%s',
                        ([loc], [city], [minprice],[area]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice == "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price>=%s and Area=%s and Atype=%s',
                        ([loc], [city], [minprice],[area],[atype]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice == "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price>=%s and Atype=%s',
                    ...
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price<=%s',
                        ([loc], [city], [maxprice]))
                    result = cur.fetchall()
                elif city != "" and minprice == "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price<=%s and Area=%s',
                        ([loc], [city], [maxprice],[area]))
                    result = cur.fetchall()
                elif city != "" and minprice == "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price<=%s and atype=%s',
                        ([loc], [city], [maxprice],[atype]))
                    result = cur.fetchall()
                elif city != "" and minprice == "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City = %s and Price<=%s and Area=%s and Atype=%s',
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price<=%s',
                        ([loc], [maxprice]))
                    result = cur.fetchall()
                elif city == "" and minprice == "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price<=%s and Area=%s',
                        ([loc], [maxprice],[area]))
                    result = cur.fetchall()
                elif city == "" and minprice == "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price<=%s and Atype=%s',
                        ([loc], [maxprice],[atype]))
                    result = cur.fetchall()
                elif city == "" and minprice == "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price<=%s and Area=%s and Atype=%s',
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price<=%s ans Area=%s and Atype=%s',
                        ([loc], [maxprice],[area],[atype]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price>=%s and Price<=%s',
                        ([loc], [minprice], [maxprice]))
                    result = cur.fetchall()
                elif city == "" and minprice != "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price>=%s and Price<=%s and Area=%s',
                        ([loc], [minprice], [maxprice],[area]))
                    result = cur.fetchall()
                elif city == "" and minprice != "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price>=%s and Price<=%s and Atype=%s',
                        ([loc], [minprice], [maxprice],[atype]))
                    result = cur.fetchall()
                elif city == "" and minprice != "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and Price>=%s and Price<=%s and Area=%s and Atype=%s',
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City=%s and Price>=%s and Price<=%s',
                        ([loc], [city], [minprice], [maxprice]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice != "" and area != "" and atype == "":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City=%s and Price>=%s and Price<=%s and Area=%s',
                        ([loc], [city], [minprice], [maxprice],[area]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City=%s and Price>=%s and Price<=%s and Atype=%s',
                        ([loc], [city], [minprice], [maxprice],[atype]))
                    result = cur.fetchall()
                elif city != "" and minprice != "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Province = % s and City=%s and Price>=%s and Price<=%s and Area=%s and Atype=%s',
                ...
                if minprice == "" and maxprice == "" and area=="" and atype=="":
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Area=%s',
                        ([city],[area]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice == "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Atype=%s',
                        ([city],[atype]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice == "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Area=%s and Atype=%s',
                        ([city],[area],[atype]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price<=%s',
                        ([city], [maxprice]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price<=%s and Area=%s',
                        ([city], [maxprice],[area]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price<=%s and Atype=%s',
                        ([city], [maxprice],[atype]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price<=%s and Area=%s and Atype=%s',
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price<=%s and Area=%s andAtype=%s',
                        ([city], [maxprice],[area],[atype]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price>=%s and Price<=%s',
                        ([city], [minprice], [maxprice]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price>=%s and Price<=%s and Area=%s',
                        ([city], [minprice], [maxprice],[area]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price>=%s and Price<=%s and Atype=%s',
                        ([city], [minprice], [maxprice],[atype]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where City = % s and Price>=%s and Price<=%s and Area=%s and Atype=%s',
                ...
                if minprice == "" and maxprice != "" and area=="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price<=%s',
                        ([maxprice]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price<=%s and Area=%s',
                        ([maxprice],[area]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price<=%s and Atype=%s',
                        ([maxprice],[atype]))
                    result = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'SELECT A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price<=%s and Area=%s and Atype=%s',
                        ([maxprice],[area],[atype]))
                    result = cur.fetchall()
                    ...
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price>=%s and Price<=%s',
                        ([minprice], [maxprice]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="" and atype=="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price>=%s and Price<=%s and Area=%s',
                        ([minprice], [maxprice],[area]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area=="" and atype!="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price>=%s and Price<=%s and Atype=%s',
                        ([minprice], [maxprice],[atype]))
                    result = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="" and atype!="":
                    cur.execute(
                        'A_ID,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where Price>=%s and Price<=%s and Area=%s and Atype=%s',
            ...
            if loc == "" and city == "" and minprice == "" and maxprice == "" and area=="":
                ...
                cur.execute(
                    'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Area=%s',([area]))
                result1 = cur.fetchall()
                ...
                if city == "" and minprice == "" and maxprice == "" and area=="":
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and Area=%s',
                        ([loc],[area]))
                    result1 = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City = %s and Area=%s',
                        ([loc], [city],[area]))
                    result1 = cur.fetchall()

                elif city != "" and minprice != "" and maxprice == "" and area=="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City = %s and Rent>=%s',
                        ([loc], [city], [minprice]))
                    result1 = cur.fetchall()
                elif city != "" and minprice != "" and maxprice == "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City = %s and Rent>=%s and Area=%s',
                        ([loc], [city], [minprice],[area]))
                    result1 = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and Rent<=%s',
                        ([loc], [maxprice]))
                    result1 = cur.fetchall()
                elif city == "" and minprice == "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and Rent<=%s and Area=%s',
                        ([loc], [maxprice],[area]))
                    result1 = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and Rent>=%s and Rent<=%s',
                        ([loc], [minprice], [maxprice]))
                    result1 = cur.fetchall()
                elif city == "" and minprice != "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and Rent>=%s and Rent<=%s and Area=%s',
                        ([loc], [minprice], [maxprice],[area]))
                    result1 = cur.fetchall()

                elif city != "" and minprice != "" and maxprice != "" and area=="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City=%s and Rent>=%s and Rent<=%s',
                        ([loc], [city], [minprice], [maxprice]))
                    result1 = cur.fetchall()
                elif city != "" and minprice != "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Province = % s and City=%s and Rent>=%s and Rent<=%s and Area=%s',
                        ([loc], [city], [minprice], [maxprice],[area]))
                    result1 = cur.fetchall()
                ...
                if minprice == "" and maxprice == "" and area=="":
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s and Area=%s',
                        ([city],[area]))
                    result1 = cur.fetchall()

                elif minprice == "" and maxprice != "" and area=="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s and Rent<=%s',
                        ([city], [maxprice]))
                    result1 = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s and Rent<=%s and Area=%s',
                        ([city], [maxprice],[area]))
                    result1 = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s and Rent>=%s and Price<=%s',
                        ([city], [minprice], [maxprice]))
                    result1 = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where City = % s and Rent>=%s and Price<=%s and Area=%s',
                        ([city], [minprice], [maxprice],[area]))
                    result1 = cur.fetchall()
                ...
                if minprice == "" and maxprice != "" and area=="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Rent<=%s',
                        ([maxprice]))
                    result1 = cur.fetchall()
                elif minprice == "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Rent<=%s and Area=%s',
                        ([maxprice],[area]))
                    result1 = cur.fetchall()
                    ...
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Rent>=%s and Rent<=%s',
                        ([minprice], [maxprice]))
                    result1 = cur.fetchall()
                elif minprice != "" and maxprice != "" and area!="":
                    cur.execute(
                        'SELECT R_ID,username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where Rent>=%s and Rent<=%s and Area=%s',
                        ([minprice], [maxprice],[area]))
                    result1 = cur.fetchall()
...
def applied_project():
    if 'loggedin' in session and session['username'] != 'admin':
      cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
      cursor.execute('SELECT P_ID,username,fullname,Pname,Flattype,Address,Features,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id where P_ID in (select P_ID from Buy_project where id=%s) ',[session['id']])
      result = cursor.fetchall()
      mysql.connection.commit()
      cursor.close()
...
def applied_room():
    if 'loggedin' in session and session['username'] != 'admin':
      cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
      cursor.execute('SELECT username,Bname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Rent,Availability,Facilities,Descr,image,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where R_ID in (select R_ID from Buy_propertyroom where id=%s) ',[session['id']])
      result = cursor.fetchall()
      mysql.connection.commit()
      cursor.close()
    ...
    if 'loggedin' in session and session['username'] != 'admin':
        ...
        if res:
            ...
            return render_template('search.html', detail="", deatail1="", detail2="", username=session['username'],
                ...
                Status = 'Not Approved'
                if len(City) > 0 and len(Age) > 0 and len(Occupation) > 0 and len(Address) > 0 and len(Landmark) > 0 and len(Zipcode) > 0 and len(Province) > 0:
                    cursor.execute('INSERT INTO Buy_propertyapt VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(id,session['id'],Age,Address,Landmark,City,Zipcode,Province,Occupation,Status))
                    mysql.connection.commit()
                    cursor.close()
    ...
    if 'loggedin' in session and session['username'] != 'admin':
        ...
        if res:
            ...
            return render_template('search.html', detail="", deatail1="", detail2="", username=session['username'],
                ...
                Status = 'Not Approved'
                if len(City) > 0 and len(Age) > 0 and len(Occupation) > 0 and len(Address) > 0 and len(Landmark) > 0 and len(Zipcode) > 0 and len(Province) > 0:
                    cursor.execute('INSERT INTO Buy_project VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(id,session['id'],Age,Address,Landmark,City,Zipcode,Province,Occupation,Status))
                    mysql.connection.commit()
                    cursor.close()
    ...
    if 'loggedin' in session and session['username'] != 'admin':
        ...
        if res:
            ...
            return render_template('search.html', username=session['username'], msg=msg,email1=session['email1'])
                ...
                Status = 'Not Approved'
                if len(City) > 0 and len(Age) > 0 and len(Occupation) > 0 and len(Address) > 0 and len(Landmark) > 0 and len(Zipcode) > 0 and len(Province) > 0:
                    cursor.execute('INSERT INTO Buy_propertyroom VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(id,session['id'],Age,Address,Landmark,City,Zipcode,Province,Occupation,Status))
                    mysql.connection.commit()
                    cursor.close()
...
def editapart(id):
    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            app.config['UPLOAD_FOLDER'] = ''
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], f_name))
            if len(apmtname) > 0 and len(plot_no) > 0 and len(area) > 0 and len(address) > 0 and len(landmark) > 0 and len(city) > 0 and len(pin) > 0 and len(Province) > 0 and len(country) > 0 and len(atype) > 0 and len(facilities) > 0:
                cur = mysql.connection.cursor()
                cur.execute("UPDATE apartmentdetail SET Aname=%s, Plot_no=%s, Area=%s, Address=%s, Landmark=%s, City=%s, Zipcode=%s, Province=%s, Country=%s, Atype=%s,RS=%s, Availability=%s,Price=%s,Facilities=%s,Descr=%s,image=%s WHERE A_ID=%s",[apmtname, plot_no, area, address, landmark, city, pin, Province, country, atype, rs, availability, Price, facilities, description, f_name,id, ])
                mysql.connection.commit()
                cur.close()
...
def editroom(id):
    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            app.config['UPLOAD_FOLDER'] = ''
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], f_name))
            if len(bname) > 0 and len(plot_no) > 0 and len(area) > 0 and len(address) > 0 and len(landmark) > 0 and len(city) > 0 and len(pin) > 0 and len(Province) > 0 and len(country) > 0 and len(facilities) > 0:
                cur = mysql.connection.cursor()
                cur.execute(
                    "UPDATE roomdetail SET Bname=%s, Room_no=%s, Area=%s, Address=%s, Landmark=%s, City=%s, Zipcode=%s, Province=%s, Country=%s, Availability=%s,Rent=%s,Facilities=%s,Descr=%s,image=%s WHERE R_ID=%s",
                    [bname, plot_no, area, address, landmark, city, pin, Province, country, availability, Rent,
...
def editproject(id):
    ...
    if request.method == 'POST':
        ...
        if extension[1] in allowed_extensions:
            ...
            cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cur.execute(
                "UPDATE projectdetail SET Pname=%s,Flattype=%s, Address=%s, Features=%s, City=%s, Zipcode=%s, Province=%s, Country=%s, Availability=%s, Facilities=%s,Descr=%s,image=%s WHERE P_ID=%s",
                [pname, flattype, address, features, city, pin, Province, country, availability,
                 facilities, description, f_name, id, ])
            mysql.connection.commit()
            cur.close()
        ...
        if session['username'] != 'admin':
            ...
            if res:
                ...
                if len(A_ID) > 0 and len(Aname) > 0 and len(Email) > 0 and len(Mobile) > 0 and len(
                    ...
                    Zipcode) > 0 and len(Province) > 0 and len(Country) > 0 and len(Occupation)>0 and len(booking_date)>0:
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        ...
                        cursor.execute(
                            ...
                            (A_ID, session['id'], Address, Landmark, City, Zipcode, Province, Country, Occupation, Slot, booking_date))
                        mysql.connection.commit()
                        cursor.close()
                        return render_template("search.html", username=session['username'], email1=session['email1'])
                else:
        ...
        if session['username'] != 'admin':
            ...
            if request.method == 'POST':
                ...
                if len(A_ID) > 0 and len(Aname) > 0 and len(Email) > 0 and len(Mobile) > 0 and len(
                    ...
                    Zipcode) > 0 and len(Province) > 0 and len(Country) > 0 and len(endtime)>0 and len(booking_date)>0:
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        msg = 'Invalid email address !'
                    elif len(Mobile) != 10:
                        msg = 'Enter 10 digit number !'
                        ...
                        cursor.execute(
                            ...
                            (A_ID, session['id'],buyer_id, Address, Landmark, City, Zipcode, Province, Country, booking_date, starttime, endtime))
                        mysql.connection.commit()
        ...
        if session['username'] != 'admin':
            ...
            if res:
                ...
                if len(R_ID) > 0 and len(Bname) > 0 and len(Email) > 0 and len(Mobile) > 0 and len(
                        Fullname) > 0 and len(
                        City) > 0 and len(Slot) > 0 and len(Address) > 0 and len(Landmark) > 0 and len(
                    Zipcode) > 0 and len(Province) > 0 and len(Country) > 0 and len(Occupation)>0 and len(booking_date)>0 and len(Room_no) >0:
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        msg = 'Invalid email address !'
                    elif len(Mobile) != 10:
                        msg = 'Enter 10 digit number !'
                        ...
                        cursor.execute(
                            ...
                            (R_ID, session['id'], Address, Landmark, City, Zipcode, Province, Country, Occupation, Slot, booking_date))
                        mysql.connection.commit()
                        cursor.close()
                        return render_template("search.html", username=session['username'], email1=session['email1'])
                else:
                ...
                if len(R_ID) > 0 and len(Bname) > 0 and len(Room_no)>0 and len(Email) > 0 and len(Mobile) > 0 and len(
                    ...
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        ...
                        cursor.execute(
                            ...
                            (R_ID, session['id'],buyer_id, Address, Landmark, City, Zipcode, Province, Country, booking_date, starttime, endtime))
                        mysql.connection.commit()
                ...
                if len(P_ID) > 0 and len(Pname) > 0 and len(Flattype)>0 and len(Email) > 0 and len(Mobile) > 0 and len(
                    ...
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        ...
                        cursor.execute(
                            ...
                            (P_ID, session['id'], Address, Landmark, City, Zipcode, Province, Country, Occupation, Slot, booking_date))
                        mysql.connection.commit()
                        cursor.close()
                        return render_template("search.html", username=session['username'], email1=session['email1'])
                else:
                ...
                if len(P_ID) > 0 and len(Pname) > 0 and len(Flattype)>0 and len(Email) > 0 and len(Mobile) > 0 and len(
                    ...
                    if not re.match(r'[^@]+@[^@]+\.[^@]+', Email):
                        ...
                        cursor.execute(
                            ...
                            (P_ID, session['id'],buyer_id, Address, Landmark, City, Zipcode, Province, Country, booking_date, starttime, endtime))
                        mysql.connection.commit()
...
def savedproperties():
    if 'loggedin' in session and session['username'] != 'admin':
        ...
        for i in r:
            cursor2.execute(
                'SELECT A_ID,Aname,fullname,username,Aname,Plot_no,Area,Address,Landmark,City,Zipcode,Province,Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating FROM apartmentdetail INNER JOIN accounts on apartmentdetail.id=accounts.id where A_ID=%s',
                ([i['A_ID']]))
            l=cursor2.fetchall()
            result=result+l
        mysql.connection.commit()
        ...
        for i in r2:
          cursor3.execute(
            'SELECT P_ID,username,Pname,fullname,Flattype,Features,Address,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,rating FROM projectdetail INNER JOIN accounts on projectdetail.id=accounts.id where P_ID=%s',
            [i['P_ID']])
          l2 = cursor3.fetchall()
          result2 = result2 + l2
        mysql.connection.commit()
        ...
        for i in r1:
          cursor4.execute(
            'SELECT R_ID,username,Bname,fullname,Room_no,Area,Address,Landmark,City,Zipcode,Province,Country,Availability,Facilities,Descr,image,Rent,rating FROM roomdetail INNER JOIN accounts on roomdetail.id=accounts.id where R_ID=%s ',
            [i['R_ID']])
          l1 = cursor4.fetchall()
          result1 = result1 + l1
        mysql.connection.commit()
...
def viewfriendsapt(id):
    if 'loggedin' in session:
        if session['username'] != 'admin':
            ...
            if r:
                cur.execute(
                    'select username,Aname,Plot_no,Area,apartmentdetail.Address,apartmentdetail.Landmark,apartmentdetail.City,apartmentdetail.Zipcode,apartmentdetail.Province,apartmentdetail.Country,Price,Atype,RS,Availability,Facilities,Descr,image,rating,Buy_propertyapt.A_ID,accounts.fullname from Buy_propertyapt INNER JOIN apartmentdetail on Buy_propertyapt.A_ID=apartmentdetail.A_ID INNER JOIN accounts on accounts.id = apartmentdetail.id where Buy_propertyapt.id=%s',
                    [id])
                res = cur.fetchall()
...
def viewfriendsroom(id):
    if 'loggedin' in session:
        if session['username']!='admin':
            ...
            r = cur.fetchall()
            if r:
              cur.execute('select Buy_propertyroom.R_ID,username,Bname,Room_no,Area,roomdetail.Address,roomdetail.Landmark,roomdetail.City,roomdetail.Zipcode,roomdetail.Province,roomdetail.Country,roomdetail.Availability,Facilities,Descr,image,Rent,rating,fullname from Buy_propertyroom INNER JOIN roomdetail on Buy_propertyroom.R_ID=roomdetail.R_ID INNER JOIN accounts on accounts.id = roomdetail.id where Buy_propertyroom.id=%s', [id])
              res=cur.fetchall()
              print(res)
...
def viewfriendsproject(id):
    if 'loggedin' in session:
        if session['username']!='admin':
            ...
            r = cur.fetchall()
            if r:
              cur.execute('select username,Pname,Flattype,Features,projectdetail.Address,projectdetail.City,projectdetail.Zipcode,projectdetail.Province,projectdetail.Country,Availability,Facilities,Descr,image,rating,fullname,Buy_project.P_ID from Buy_project INNER JOIN projectdetail on Buy_project.P_ID=projectdetail.P_ID INNER JOIN accounts on accounts.id = projectdetail.id where Buy_project.id=%s', [id])
              res=cur.fetchall()
